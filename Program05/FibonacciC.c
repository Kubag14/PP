...
void fbn_i(long int fibonacci[], unsigned n)
{ unsigned i;
    fibonacci[0] = 0, fibonacci[1] = 1, fibonacci[2] = 1;// wartosci poczatkowe (rownoczesnie trzy pierwsze wyrazy ciagu)
    for (i = 3; i < n+1; ++i) // petla powtarza sie, poki i jest mniejsze od n - 1, gdzie n jest numerem liczby w ciagu
    {
...
int main()
{
    ...
    do {
        printf("Podaj rozmiar tablicy/maksymalna liczbe F - liczbe naturalna t = ");
        scanf("%u", &t); // wczytanie rozmiaru tablicy t
    } while (t < 1); // wprowadzone t musi byc rowne co najmniej 1

    long int fibonacci[t]; // definiujemy tablice o rozmiarze t
